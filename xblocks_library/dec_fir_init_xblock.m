...
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dec_fir_init_xblock(blk, n_inputs, coeff, n_bits, quantization, add_latency, mult_latency, coeff_bit_width, coeff_bin_pt, adder_tree_hdl)
...
disp(num_fir_col);
if coeff_round(1:int32(length(coeff)/2)) == coeff_round(length(coeff):-1:int32(length(coeff)/2)+1)
...
if coeff_sym
    for k=1:n_inputs,
        xlsub2_fir_col{num_fir_col*2-1,2*n_inputs+k*2-1}.bind(xlsub2_fir_col{num_fir_col*2,k*2-1});
        xlsub2_fir_col{num_fir_col*2-1,2*n_inputs+k*2}.bind(xlsub2_fir_col{num_fir_col*2,k*2});
    end
end
...
for i=1:num_fir_col,
    ...
    if coeff_sym
        if i< num_fir_col,
            for k=1:n_inputs,
                xlsub2_fir_col{i*2-1,2*n_inputs+k*2-1}.bind(xlsub2_fir_col{i*2+2,2*n_inputs+k*2-1});
                xlsub2_fir_col{i*2-1,2*n_inputs+k*2}.bind(xlsub2_fir_col{i*2+2,2*n_inputs+k*2});
            end
            
            for j=1:n_inputs,
                xlsub2_fir_col{i*2+1,j*2-1}.bind(xlsub2_fir_col{i*2,j*2-1});
                xlsub2_fir_col{i*2+1,j*2}.bind(xlsub2_fir_col{i*2,j*2});
            end
        end
        ...
        xlsub2_real_sum_inports{i+1}.bind(xlsub2_fir_col{i*2,n_inputs*sym+1});
        xlsub2_imag_sum_inports{i+1}.bind(xlsub2_fir_col{i*2,n_inputs*sym+2});
    else
        xlsub2_real_sum_inports{i+1}.bind(xlsub2_fir_col{i+1,n_inputs*sym+1});
        xlsub2_imag_sum_inports{i+1}.bind(xlsub2_fir_col{i+1,n_inputs*sym+2});
    end
    
    
    if coeff_sym
        xlsub2_fir_col_sub{i} = xBlock(struct('source',str2func([fir_col_type,'_init_xblock']), 'name', [fir_col_type,num2str(i)]), ...
                                 {[blk,'/',fir_col_type,num2str(i)], n_inputs, coeff(i*n_inputs:-1:(i-1)*n_inputs+1), ...
...
xlsub2_imag_sum_sub = xBlock(struct('source',str2func('adder_tree_lib_init_xblock'), 'name', 'imag_sum'), ...
                         {[blk,'/imag_sum'],'n_inputs',num_fir_col,...
                             'latency', add_latency, ...
                             'first_stage_hdl', 'off'} , ... %% for now just set them to off, ignore 'adder_tree_hdl'
                         xlsub2_imag_sum_inports, ...
                         {[], xlsub2_imag_sum_out2});

% block: untitled/dec_fir/real_sum
xlsub2_real_sum_sub = xBlock(struct('source', str2func('adder_tree_lib_init_xblock'), 'name', 'real_sum'), ...
                         {[blk,'/real_sum'], 'n_inputs',num_fir_col,...
                             'latency', add_latency, ...
                             'first_stage_hdl', 'off'} , ... %% for now just set them to off, ignore 'adder_tree_hdl'
                         xlsub2_real_sum_inports, ...
...
xlsub2_ri_to_c = xBlock(struct('source', str2func('ri_to_c_init_xblock'), 'name', 'ri_to_c'), ...
                        [], ...
